...
namespace NHST.manager
{
    public partial class Noti_app_list : System.Web.UI.Page
    {
        ...
        private void LoadData()
        {
            ...
            var total = AppPushNotiController.GetTotal();
            var la = AppPushNotiController.GetAll();
            pagingall(la, total);
        }

        #region Pagging
        public void pagingall(List<tbl_AppPushNoti> acs, int total)
        {
            int PageSize = 20;
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    hcm.Append("<td>" + item.AppNotiTitle + "</td>");
                    hcm.Append("<td>" + item.AppNotiMessage + "</td>");
